...
HU = dict(
 ...
 s_proj='Projektek',
 proj=[('Fanni Lashes — időpontfoglaló webalkalmazás','Angular · Supabase · PostgreSQL',['Éles, ügyfélnek készült full-stack projekt: műszempilla-szalon weboldala online időpontfoglalóval és teljes admin felülettel (foglalások, árlista, nyitvatartás, galéria, statisztikák). Négylépéses foglalási varázsló valós idejű szabadidőpont-számítással; a foglalási logika PL/pgSQL függvényekben él, dupla foglalás elleni védelem adatbázis-szintű exclusion constrainttel, Row Level Security minden táblán — dedikált backend szerver nélkül.']),
       ('IAKK Dashboard','PHP',['Az IAKK Dashboard egy olyan felület, amely nagyon hasonlít a KRÉTA-hoz — annak egy továbbgondolt verziója. A Dashboard képes kezelni a diákokkal kapcsolatos információkat (pl. jelenléti mátrix, szabadság, táppénz). Beépített fizetési rendszerrel rendelkezik, és különféle dokumentumok exportálása is lehetséges. PHP nyelven szereztem tapasztalatot ezen a projekten.']),
...
EN = dict(
 ...
 s_proj='Projects',
 proj=[('Fanni Lashes — online booking web application','Angular · Supabase · PostgreSQL',['A live, client-facing full-stack project: a lash studio website with an online booking system and a full admin panel (bookings, price list, opening hours, gallery, statistics). Four-step booking wizard with real-time availability calculation; the booking logic lives in PL/pgSQL functions, with database-level double-booking protection via an exclusion constraint and Row Level Security on every table — without a dedicated backend server.']),
       ('IAKK Dashboard','PHP',['The IAKK Dashboard is an interface very similar to KRÉTA (the Hungarian school administration system) — a more advanced take on it. The dashboard manages student-related information (e.g. attendance matrix, holidays, sick leave). It has a built-in payment system and can export various documents. I gained PHP experience on this project.']),
